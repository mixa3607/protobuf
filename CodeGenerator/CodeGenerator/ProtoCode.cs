...
namespace SilentOrbit.ProtocolBuffers
{
    internal static class ProtoCode
    {
        ...
        public static void Save(ProtoCollection file, Options options)
        {
            ...
            if (Directory.Exists(csDir) == false && !string.IsNullOrWhiteSpace(csDir))
            {
...
then write the code and the changes in a separate file.");
                ...
                foreach (ProtoMessage m in file.Messages.Values)
                {
                    if (ns != m.CsNamespace)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(options.BaseNamespace))
                        {
                            cw.Bracket("namespace " + options.BaseNamespace + '.' + m.CsNamespace);
                        }
                        else
                        {
                ...
                foreach (ProtoEnum e in file.Enums.Values)
                {
                    if (ns != e.CsNamespace)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(options.BaseNamespace))
                        {
                            cw.Bracket("namespace " + options.BaseNamespace + '.' + e.CsNamespace);
                        }
                        else
                        {
            ...
            if (!options.NoGenerateSerializer)
            {
